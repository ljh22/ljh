...
const Easing = {
    Power: {
        ...
        InOut: (power) => {
            ...
            return (t) =>
                t < 0.5 ? Math.pow(t * 2, power) / 2 : (1 - Math.abs(Math.pow(t * 2 - 1 - 1, power))) / 2 + 0.5;
        },
    },
...
class Controls {
    constructor(game) {
        ...
        this.edges = new THREE.Mesh(new THREE.BoxBufferGeometry(0.95, 0.95, 0.95), helperMaterial.clone());
    ...
    getPiecePosition(piece) {
        let position = new THREE.Vector3().setFromMatrixPosition(piece.matrixWorld).multiplyScalar(3);
